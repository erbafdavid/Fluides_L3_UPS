...
%\subsubsection{Bilans locaux}
%-----------------------------------------------------------------------------------------
\begin{frame}{Bilan local de quantité de mouvement }
%-----------------------------------------------------------------------------------------
...
\subsubsection{Analyse dimensionnelle de l'equation de Navier-Stokes}

\begin{frame}{Analyse dimensionnelle de l'equation de Navier-Stokes} 

\small

En considérant toujours le cas d'un liquide incompressible (non pesant), cherchons à estimer l'{\em ordre de grandeur} des termes de l'équation de Navier-Stokes pour un écoulement autour d'un obstacle.
...
Ordres de grandeurs : 
\begin{center}
	\begin{tabular}{lp{3mm}lll}
		$\color{rouge} (x,y,z) = {\mathcal O} (L)$  & & $L$ :  &  \dotfill\ échelle caractéristique de longueur & (fixée par la géométrie)
		\\
		$\color{rouge} t =  {\mathcal O} (T)$ &  & $T$ : & \dotfill\ de temps 	&(fixé ou inconnu)
		\footnote{non pertinent si l'écoulement est stationnaire}  
		\\
		$\color{rouge} |\vec{u}| =  {\mathcal O} (U)$ &  & $U$ : & \dotfill\ échelle caractéristique (ou {\em jauge}) de vitesse &  (fixé par les cond. lim.)
		\footnote{Rem : pour le écoulements externes, $U$ est fixé par les cond. limites et $\delta p$ est inconnu; pour les écoulements internes c'est souvent l'inverse ; cf. chap. 4} 
		\\
		$\color{rouge} p =   {\mathcal O} (p_0)$ &  & $p_0$ : & \dotfill\  échelle caractéristique de pression & (fixé) 
		\\
		$\color{rouge} p' = (p-p_0) = {\mathcal O} (\delta p)$ & & $\delta p : &    \dotfill\  jauge des \underline{variations de} pression & (inconnu)$^{2}$ 
	\end{tabular}
\end{center} 
}
...
%{\tiny 
%\begin{tabular}{lll} 
%Echelle caractéristique de longueur : & $L$ \qquad & (fixé par la géométrie) \\
%Echelle caractéristique de temps  : & $T$ & (fixé par les cond. limites ou inconnu)\footnote{non pertinent si l'écoulement est stationnaire} \\
%Echelle (ou jauge) de vitesse : & $U$ & (fixé par les cond. limites)
%\footnote{Rem : pour le écoulements externes, $U$ est fixé par les cond. limites et $\delta p$ est inconnu; pour les écoulements internes c'est souvent l'inverse ; cf. chap. 4} \\
%\qquad (En posant  $(p+\rho g z) = p_0  + \hat{p}'$ ):\\
%Echelle (ou jauge) de pression : & $p_0$ &  (fixé par les cond. limites) \\
%Echelle (ou jauge) de \underline{variation} de pression \underline{motrice} : & $ [\hat{p}'] =  \delta p &  (inconnu)$^{1}$ \\
%\end{tabular}
%}
...
Considérons maintenant le cas des écoulements de  {\em gaz parfaits} et cherchons à estimer l'{\em ordre de grandeur} 
des termes de l'équation de continuité.
...
\begin{center}
	\begin{tabular}{lp{3mm}lll}
		...
		$\color{rouge} |\vec{u}| =  {\mathcal O} (U)$ &  & $U$ : & \dotfill\ échelle caractéristique (ou {\em jauge}) de vitesse &  (fixé par les cond. lim.)
		\\
		$\color{rouge} \rho =   {\mathcal O} (\rho_0)$ &  & $p_0$ : & \dotfill\  échelle caractéristique de masse volumique & (fixé) 
		\\
		$\color{rouge} \rho' = (\rho-\rho_0) = {\mathcal O} (\delta p)$ & & $\delta p : &    \dotfill\  jauge des \underline{variations de} masse volumique & (inconnu)
		\\
		$\color{rouge} \divergence( \vec{u} ) = {\mathcal O}( D) $ & &  $D$ : &  \dotfill\ jauge de la divergence & (inconnu)
		\footnote{N'écrivons pas trop vite  $D = U/L$ car les 3 termes apparaissant dans la divergence peuvent se compenser !} 
	\end{tabular}
\end{center} 
}
...
%{\tiny
%\begin{tabular}{lll} 
%\qquad ( en Posant : \qquad  $\rho = \rho_0 + \rho'$)\\
%Echelle de masse volumique :  & $\rho_0$ \qquad & (fixé par les C.L.) \\
%Echelle (ou jauge) de \underline{variation de} masse volumique & \delta \rho &  (inconnu) \\
%Ordre de grandeur de la divergence : & $[\divergence( \vec{u} )] & (inconnu !)
%\footnote{N'écrivons pas trop vite  $[\divergence( \vec{u} )] = U/L$ car les 3 termes apparaissant dans la divergence peuvent se compenser !} 
%\end{tabular}
%}
...
\begin{array}{lcccccc} 
\displaystyle
 &\dpdt{\rho} & + &  \myvec{u} \cdot  \myvec{grad} \rho  & + & \rho \divergence(\myvec{u}) 
 \\
 \pause
\displaystyle
%\mbox{(Terme :)} 
&
\left[I\right] && \left[ C \right] && \left[D\right] \\
\pause
\displaystyle
\mbox{(Ordre de grandeur :)} &
\frac{\delta \rho}{T}  && \frac{U \delta \rho}{L} && \rho_0 D$
\end{array}
$$
...
- Ecoulement adiabatique : $\delta p / \delta  \rho \approx (\partial p / \partial \rho)_s = c_0^2 \quad \rightarrow {\color{red} \delta \rho = \frac{1}{c_0^2}  \delta p}$
\pause

- Régime inertiel : $\delta p = \rho_0 U^2 \quad \rightarrow {\color{red} \frac{\delta \rho}{\rho_0}  = \frac{U^2}{c_0^2} = M^2 \mbox{ Nombre de Mach} } $ 

\pause
- Oublions le terme $[I]$ qui n'est dominant que dans le régime acoustique ( cf. chapitre 10).


\smallskip \pause

le PMD indique alors que $[D] = [C]$ 

\smallskip \pause

 $\longrightarrow \quad \color{rouge} D  = M^2 U/L$.
...
{\bf Premier cas :}  Si $M \ll 1$, alors l'équation de continuité se simplifie en {\color{red}  $\divergence( \vec{u} ) = 0$.}

\pause \smallskip
Dans ce cas, un gaz parfait ({\em fluide compressible}) s'écoule de manière isovolume. 

 \smallskip

On parle de {\color{red} Régime d'écoulement incompressible}.

\pause
\smallskip
$=>$ On peut de nouveau utiliser l'équation de Navier-Stokes incompressible et toutes les discussions des deux paragraphes précédents 
(simplifications, régimes d'écoulement) restent valables !

\pause
\bigskip

{\bf Second cas :}  Si $M = {\cal O}(1)$ (a fortiori si $M \gg 1$), alors l'équation de continuité ne se simplifie pas.

\smallskip

On parle de  {\color{red} Régime d'écoulement compressible} (cf. chapitres 11-12).
